...
def load_zip_file(file,fileNameRegExp='',allEntries=False):
    ...
    # 读取一个zip压缩包里所有文件的名字
    for name in archive.namelist(): #z.namelist() 会返回压缩包内所有文件名的列表。
        addFile = True
        keyName = name
        if fileNameRegExp!="":
            m = re.match(fileNameRegExp,name)
...
def main_evaluation(p, default_evaluation_params_fn, validate_data_fn, evaluate_method_fn, num, show_result=True, per_sample=True):
    """
    ...
    try:
        ...
        evalData = evaluate_method_fn(p['g'], p['s'], evalParams, num)
        print("===============ENDING===========")
